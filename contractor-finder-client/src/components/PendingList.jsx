...
export default function PendingList() {
  ...
  const displayUsers       = (activeTab !== "contractors" ? pending.users       : []).filter(filterFn);
  const displayContractors = (activeTab !== "users"       ? pending.contractors : []).filter(filterFn);
  ...
  return (
    <>
      <style>{`
        ...
        .pl-search:focus  { border-color:rgba(124,58,237,0.6) !important; outline:none; }
      `}</style>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center", padding:"60px 20px", color:"rgba(255,255,255,0.25)", fontSize:15, fontWeight:700 }}>
          ...
          {search ? "No results match your search." : "No pending requests! All caught up."}
        </div>
      ) : (
